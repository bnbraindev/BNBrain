...
export function DappSecurityCard({ data, locale = 'en' }: { data: DappSecurityCardData; locale?: string }) {
  const zh = locale === 'zh';
    ...
    <Card className={safe ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-amber-500/30 bg-amber-500/5'}>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-sm">
          ...
          {data.projectName || (zh ? 'dApp 安全' : 'dApp Security')}
          {data.trustList && <Badge variant="outline" className="ml-auto text-xs border-emerald-500/30 text-emerald-500">{zh ? '已信任' : 'Trusted'}</Badge>}
          {data.isAudit && <Badge variant="outline" className="text-xs border-blue-500/30 text-blue-500">{zh ? '已审计' : 'Audited'}</Badge>}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        {data.auditInfo && data.auditInfo.length > 0 && (
          <div className="space-y-1">
            <p className="text-xs font-medium flex items-center gap-1"><FileCheck className="size-3" /> {zh ? '审计记录' : 'Audit Records'}</p>
        ...
        {data.contractsSecurity && data.contractsSecurity.length > 0 && (
          <div className="space-y-1">
            <p className="text-xs font-medium">{zh ? '合约' : 'Contracts'}</p>
            {data.contractsSecurity.flatMap((cs) =>
              cs.contracts.map((c, i) => (
                <div key={`${cs.chainId}-${i}`} className="flex items-center gap-2 text-xs">
                  ...
                  {c.isOpenSource && <Badge variant="outline" className="text-xs">{zh ? '开源' : 'Open Source'}</Badge>}
                </div>
              ))
            )}
          </div>
